...
EXPECTED_MANUAL_VULGAR = 38
EXPECTED_RAW_GENERATED = 8_862
EXPECTED_SUBSUMED_GENERATED = 1_211
EXPECTED_EMITTED_GENERATED = 7_651
EXPECTED_CANONICAL_VULGAR = 7_689
...
def generated_terms(manual_terms: set[str]) -> tuple[list[str], set[str]]:
    ...
    emitted: list[str] = []
    skipped: set[str] = set()
    for term in sorted(raw, key=lambda value: (len(value.split()), value)):
        tokens = term.split()
        subsumed = any(
...
def expected_source(source: str) -> tuple[str, int]:
    ...
    generated_block = [
        ...
        "# raw=8862, subsumed=1211, emitted=7651, total-canonical-vulgar=7689",
