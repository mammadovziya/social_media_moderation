-- Admit the HATE local blocked-term category into decision provenance.
--
-- The gateway can now classify a local blocked term as hate speech, which outranks vulgarity so an
-- ethnic slur is never audited as mere profanity. Every layer that enumerated the governed local
-- categories predates that category and rejected it: the image audit CHECK constraint refused the
-- value outright, and both provenance triggers fell through to their "requires a governed local
-- category" branch. A HATE local terminal block resolves to SAFETY, exactly as VULGAR does, so
-- each site gains the value beside VULGAR rather than gaining a new rule.
...
COMMENT ON COLUMN moderation_image_decision_audit_events.local_policy_violation IS
    'Governed local blocked-term category behind a terminal local decision: VULGAR, HATE, '
    'POLITICAL_CONTENT, or legacy OTHER. HATE outranks VULGAR and resolves to SAFETY.';
