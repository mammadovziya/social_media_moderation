ALTER TABLE moderation_image_decision_audit_events
    ...
    DROP CONSTRAINT moderation_image_decision_audit_final_reason_enum,
    ...
    ADD CONSTRAINT moderation_image_decision_audit_restricted_political_entity_enum
        CHECK (restricted_political_entity IS NULL OR restricted_political_entity IN (
    ...
    ADD CONSTRAINT moderation_image_decision_audit_configuration_coherence
        CHECK (
            ...
            OR (
                ...
                AND (
                    (provenance_schema_version = 'image-decision-provenance-v4'
                        AND decision_configuration_version = 'image-decision-config-v3')
                    OR
                    (provenance_schema_version = 'image-decision-provenance-v3'
                        AND decision_configuration_version = 'image-decision-config-v2')
                    OR
                    (provenance_schema_version NOT IN (
                            'image-decision-provenance-v3',
                            'image-decision-provenance-v4'
                        )
...
COMMENT ON COLUMN moderation_image_decision_audit_events.restricted_political_entity IS
    'Governed political-entity signal; required for v4 model-evaluated audit events';
...
ALTER TABLE moderation_username_decision_audit_events
    ADD COLUMN restricted_political_entity VARCHAR(16),
    ADD CONSTRAINT moderation_username_audit_restricted_political_entity
        CHECK (restricted_political_entity IS NULL OR restricted_political_entity IN (
            'NONE', 'PRESIDENT', 'MINISTER', 'YAP', 'MULTIPLE', 'POSSIBLE'
        )),
...
CREATE FUNCTION enforce_username_v3_policy_signals()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE TRIGGER moderation_username_audit_v3_policy
BEFORE INSERT ON moderation_username_decision_audit_events
FOR EACH ROW
EXECUTE FUNCTION enforce_username_v3_policy_signals();
